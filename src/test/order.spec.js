...
describe('prove the right functionality of the order endpoint', () => {
  ...
  test('Create a client, return it and create an order with that client', async () => {
    const client = await request(app).post(GLOBAL_ROUTE_CLIENT).send({
      name: 'Zain',
      lastName: 'Kerr',
      inDebt: 1000
    })
  ...
  test('Get a existent client and create an order with it', async () => {
    const client = await request(app).get(`${GLOBAL_ROUTE_CLIENT}/query`).send({
      name: 'Juan',
      lastName: 'Gonzales',
      inDebt: 1000
    })
  ...
  test('Get an existent order and change its state', async () => {
    ...
    const order = await request(app).post(GLOBAL_ROUTE_ORDER).send({
      client: client.body,
      stateCounter: 1
    })
